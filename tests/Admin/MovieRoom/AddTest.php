...
final class AddTest extends TestCase
{
    ...
    private function getAttemptCallingResponse(array $formParams = []) : \Illuminate\Testing\TestResponse
    {
        ...
        return $this->actingAs($this->user, 'admin')
            ->followingRedirects()
            ->from('admin/movie-rooms/add')
            ->post('admin/movie-rooms/add', $formParams)
            ->assertStatus(200);
    }
     ...
     * @return void
     */
    public function testSuccess(array $formParams) : void
    {
        $response = $this->getAttemptCallingResponse($formParams);

        $responseString = $response->getContent();
        $crawler = new Crawler($responseString);

        // On vérifie le message Flash
        $expectedMessage = sprintf('La salle %s a été créée.', $formParams['name']);
        $flashMessage = $crawler->filter('p.with-color.with-color-green')->text();
        $this->assertEquals($expectedMessage, $flashMessage);

        // Vérification des données
        $movieRoomCreated = MovieRoom::latest('id')->first();
        $expectedData = [
            'public_id' => $formParams['public_id'],
            'name' => $formParams['name'],
            'floor' => $formParams['floor'],
            'nb_places' => $formParams['nb_places'],
            'nb_handicap_places' => $formParams['nb_handicap_places'],
        ];
        $realData = [
            'public_id' => $movieRoomCreated->public_id,
            'name' => $movieRoomCreated->name,
            'floor' => $movieRoomCreated->floor,
            'nb_places' => $movieRoomCreated->nb_places,
            'nb_handicap_places' => $movieRoomCreated->nb_handicap_places,
        ];
    ...
    public function testValidationFailure(array $formParams, array $errors) : void
    {
        ...
        $movieRoomTable = with(new MovieRoom())->getTable();
        $this->assertDatabaseMissing($movieRoomTable, [
            'public_id' => $formParams['public_id'] ?? null,
            'name' => $formParams['name'] ?? null,
        ]);
    }
